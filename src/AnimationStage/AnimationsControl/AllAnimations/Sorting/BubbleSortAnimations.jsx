...
const BubbleSortAnimations = ({
  ...
  function animateCompare(box1, box2, totalFrames) {
  ...
  function animateSwap(box1, box2, totalFrames) {
    return new Promise((resolve) => {
      const canvas = canvasRef.current;
      const context = canvasRef.current.getContext("2d");
      const distanceApart = Math.abs(box1.x - box2.x);
      const incramentValue = distanceApart / totalFrames;
  ...
  async function play() {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      for (let j = 0; j < n - 1 - i; j++) {
        ...
        await animateCompare(arr[j], arr[j + 1], totalFrames);
        if (arr[j].value > arr[j + 1].value) {
          //play the swap animation and swap elements
          await animateSwap(arr[j], arr[j + 1], totalFrames);
          let tmpArr = arr;
          let temp = tmpArr[j];
          tmpArr[j] = tmpArr[j + 1];
          tmpArr[j + 1] = temp;
